...
class CropParametersNew(object):
    ...
    def initial(self):
        ...
        arr_zeros = np.zeros((self.model.nFarm, self.model.nCrop, self.model.nCell))
        ...
        if len(self.model.crop_parameters_to_read) > 0:
            for param in self.model.crop_parameters_to_read:
                ...
                if read_from_netcdf:
                    ...
                    d = d[self.model.landmask_crop].reshape(self.model.nCrop,self.model.nCell)
                    vars(self.model)[param] = np.broadcast_to(d, (self.model.nFarm, self.model.nCrop, self.model.nCell))                    
                else:
                    try:
                        ...
                        vars(self.model)[param] = np.broadcast_to(
                            ...
                            (self.model.nFarm, self.model.nCrop, self.model.nCell)
                        )
