...
class SoilProfileGrid(object):
    def __init__(self, SoilProfile_variable):
        self.var = SoilProfile_variable
        self.var.dz = self.var._configuration.SOIL_HYDRAULIC_PARAMETERS['dzSoilCompartment']  # TODO: SOIL_HYDRAULIC_PARAMETERS -> SOIL_PROFILE
...
class SoilProfilePoint(object):
    def __init__(self, SoilProfile_variable):
        ...
        soil_profile = read_csv(
            self.var._configuration.SOIL_PROFILE['soilProfileFile'],
            delimiter='\s+|\t',
            header=None,
        ...
        soil_hydrology = read_csv(
            str(self.var._configuration.SOIL_HYDRAULIC_PARAMETERS['soilHydrologyFile']),
            delimiter='\s+|\t',
            header=None,
...
class SoilHydraulicParametersPoint(SoilHydraulicParameters):
    ...
    def read_soil_hydraulic_parameters(self):
        ...
        for value, var_name in soil_hydraulic_parameters.items():
            d = np.broadcast_to(value[:,None], self.var.nLayer, self.var.nCell)
...
class SoilHydraulicParametersGrid(SoilHydraulicParameters):
    def __init__(self, SoilHydraulicParametersGrid_variable, config_section_name):
        self.var = SoilHydraulicParametersGrid_variable
    ...
    def read_soil_hydraulic_parameters(self):        
        ...
        landmask_layer = np.broadcast_to(
        ...
        for param, var_name in soil_hydraulic_parameters.items():
            ...
            d = file_handling.netcdf_to_arrayWithoutTime(
                self.var._configuration.SOIL_HYDRAULIC_PARAMETERS['soilHydraulicParametersNC'],
                var_name,
                cloneMapFileName=self.var.cloneMapFileName
            )
            d = d[landmask_layer].reshape(self.var.nLayer,self.var.nCell)
            vars(self.var)[param] = np.broadcast_to(
                d[None,None,:,:],
                (self.var.nFarm, self.var.nCrop, self.var.nLayer, self.var.nCell)
            )
...
#     def __init__(self, SoilHydraulicParameters_variable, config_section_name):
#         self.var = SoilHydraulicParameters_variable
#         SoilProfileGrid(SoilHydraulicParameters_variable)
